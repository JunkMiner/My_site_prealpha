...
// the formats of notes pages are as below
    if (dq('.mdtemp')) {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            let pcodes = dqAll('#container code');

            pcodes.forEach((el) => {
                if (el.className)
                    el.className = `language-${el.className}`;
            ...
            for (let k = 0; k < pcodes.length; k++) { // add the elements recursively from bottom to top
                ...
                if (pcodes[k].parentNode.childElementCount === 1 && pcodes[k].textContent === pcodes[k].parentNode.textContent) {
                    ...
                    pcodes[k].parentNode.innerHTML = `<div class="title-bar"><div class="title-bar-text">${pcodes[k].classList[0].split('-')[1]}</div><div class="title-bar-controls"></div></div><div class="content-bar">${pcodes[k].parentNode.innerHTML}</div>`;
